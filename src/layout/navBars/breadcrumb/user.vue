...
<!--    <el-select v-model="userInfo.currentEstateId" @change="changeEstate" size="default" filterable>-->
<!--      <el-option-group-->
<!--          v-for="group in groupList"-->
<!--          :key="group.baseGroup.id"-->
<!--          :label="group.baseGroup.shortName"-->
<!--      >-->
<!--        <el-option-->
<!--            v-for="item in group.estateList"-->
<!--            :key="item.id"-->
...
export default defineComponent({
    ...
    const groupList = computed(() => {
      return userInfos.userEstateList
    })
    ...
    // 获取用户的小区列表
    const getGroupList = () => {
      // getGroupListForUser().then((res: any) => {
      //   const list = res.data.list;
      //   store.commit('userInfos/setUserEstateList', res.data.list);
      //   if ((userInfo.currentEstateId == 0 || userInfo.currentGroupId == 0) && list.length > 0 && list[0].estateList.length > 0) {
      //     changeCurrentEstate({id: list[0].estateList[0].id})
      //   }
      // });
    }
    // 切换小区
    const changeEstate = (val: any) => {
      // changeCurrentEstate({id: val}).then((res: any) => {
      //   // 存储用户信息到浏览器缓存
      //
      //   userInfo.currentEstateId = val;
